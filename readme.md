...
- **5-day forecast** — Open-Meteo (no API key), uses config lat/lon, browser geolocation, or a city/ZIP you type in edit mode; each day shows a [Meteocons](https://meteocons.com) sky icon, high/low, precipitation chance, and dominant wind direction (arrow + compass point) with peak speed in knots
